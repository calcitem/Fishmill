...
struct BoolConditions
{
    ...
    {
        values.resize(size, defaultValue), binary.resize(size, 0);
    }
    void set();
...
// Use polymorphism to accomodate Entry of different types in the same vector
    ...
    int add(const SetRange &, std::string &&)
    {
        return 0;
    }
...
public:
    ...
    static void init()
    {
        for (auto &e : instance().list) e->init_option(); read_options();
    } // Deferred, due to UCI::Options access
    static void read_options()
    {
        for (auto &e : instance().list) e->read_option();
    }
