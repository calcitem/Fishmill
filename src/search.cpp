...
void Thread::search()
{
    ...
    // Evaluation score is from the black point of view
    contempt = (us == BLACK ? make_score(ct, ct / 2)
                : -make_score(ct, ct / 2));
...
Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth, bool cutNode)
{
    ...
    bool ttHit, ttPv, formerPv, improving, didLMR, priorCapture;
    bool capture, doFullDepthSearch, moveCountPruning, ttCapture, singularLMR;
    Piece movedPiece;
    ...
    if (!rootNode) {
        ...
        if (Threads.stop.load(std::memory_order_relaxed)
            || pos.is_draw(ss->ply)
            || ss->ply >= MAX_PLY)
            return (ss->ply >= MAX_PLY /* && !ss->inCheck */) ? evaluate(pos)   // TODO
    ...
    if (ttPv && depth > 12 && ss->ply - 1 < MAX_LPH && !pos.captured_piece() && is_ok((ss - 1)->currentMove))      // TODO
